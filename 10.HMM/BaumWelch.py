...
def baum_welch(observation, state_size, observation_size, epsilon=1e-8, max_steps=500):
    """
    Given the observation of an HMM,
    return the parameter of learnt HMM
    """
    # initial parameters
    ...
    state2state /= state2state.sum(axis=0, keepdims=True)
    state2observation /= state2observation.sum(axis=0, keepdims=True)
    ...
    for step in range(max_steps):
        ...
        state2state = state_trans_prob.sum(axis=0) / state_prob[: -1].sum(axis=0)[:, None]
        state2observation = ((observation[:, None] == np.arange(observation_size)[None, :])[:, None, :] * state_prob[:, :, None]).sum(axis=0) / state_prob.sum(axis=0)[:, None]
        initial_state = state_prob[0]
